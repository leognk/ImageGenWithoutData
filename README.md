...
This possibility is supported by the strong ability of LLMs to compress images and audio despite being trained solely on text, surpassing domain-specific compressors like PNG.[^1] Surprisingly, by only looking at text, LLMs become general-purpose compressors. Even though these modalities are projections of the same world, they are still vastly different from each other. This ability is probably due to in-context compression. This raises the question: "Could there be a universal form of data, even more complex than human language, that could create the ultimate universal compressor by learning something like in-context compression?"
